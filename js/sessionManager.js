   ...
   // checkLogged();
    ...
    if(profile!=null){
       ...
       // $('#navbar').append('<button id="logout" class="col-2 btn shadow float-right mt-2 " style="height:30px" onClick="loggout();"><span class="align-middle text-white">Logga ut</span></button>')
        ...
        $('#sidebar').append('<div class="d-block mb-0 pt-4 w3-hide-large text-center  bg-light">Inloggad som:'+profile.name+'<button id="logout" class=" d-block btn logout-btn shadow w-100" onClick="loggout();"><span class=" text-white">Logga ut</span></button></div>')
    }
